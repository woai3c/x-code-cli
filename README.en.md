...
**Open-source AI coding agent for your terminal — read, modify, debug, and build with natural language.**
...
X-Code CLI (`xc`) is a terminal-based AI coding agent that interacts with your codebase through natural language. It is not locked to a single vendor — it supports Anthropic, OpenAI, DeepSeek, Google, Alibaba, xAI, Zhipu, and Moonshot out of the box, plus any OpenAI-compatible endpoint.

```text
Natural language prompt
        ↓
   X-Code CLI (14 built-in tools + sub-agents + MCP extensions)
        ↓
   Code edits / Shell execution / Search / Planning / Task tracking / …
```

## Why X-Code CLI?

- **Multi-model freedom** — 8 providers + custom OpenAI-compatible endpoints, switch with `/model`
- **14 built-in tools** — file I/O, shell execution, code search, web fetch, task tracking, and more
- **Sub-agent delegation** — research, review, planning, and verification run in isolated context
- **Plan mode** — design a plan first, execute only after approval
- **Plugin + MCP ecosystem** — byte-compatible with Claude Code plugins, install directly
...
- **14 built-in tools** — file I/O, shell execution, code search (Grep / Glob), web fetch, sub-agent delegation, todo tracking, and more
...
**Claude Code compatibility**: the plugin loader recognises both `.x-code-plugin/plugin.json` and `.claude-plugin/plugin.json`, so plugins authored for Claude Code install directly. The MCP config schema is identical. First-run auto-subscribes to `anthropic-marketplace`.
