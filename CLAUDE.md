...
`LoopState` is reused across submits within one CLI session (see `loopStateRef` in `use-agent.ts`). It carries `messages`, accumulated `tokenUsage`, `recentToolCalls` (the loop-guard window), `systemPromptCache`, the deferred-tool state (`deferredCatalog` + `activatedTools`), and `systemPromptBlocks` — snapshots of the capability blocks embedded in the cached prompt, used by the `/usage` context-breakdown estimator so a mid-session `/skill` or `/mcp` refresh can't skew the per-category split. The per-invocation turn counter is **not** on `LoopState` — it's a local in `agentLoop` and surfaces via the return value, so re-entering the function (next user submit) starts at 0. Main interactive mode passes no `maxTurns` (unlimited; press Esc to stop); `--print` and sub-agents pass per-call caps.

**`systemPromptCache` must remain byte-stable between invalidation events.** OpenAI-compatible providers (DeepSeek / Moonshot / Alibaba / Zhipu / xAI) auto-cache stable prefixes. `buildSystemPrompt` runs on the first turn and the cached string is reused until it is explicitly invalidated — by a model switch or a tool-surface change such as a `/skill` / `/mcp` refresh (`invalidateModelDependentState` / `invalidateToolSurfaceState` in `cli/src/ui/hooks/model-switch-state.ts`), or by a permission-mode flip (tool-execution nulls the cache so the plan-mode overlay folds into a fresh prompt). Each invalidation pays exactly one cache miss, and `core/src/agent/cache-stats.ts` records the reason via `markExpectedCacheMiss` so legitimate refreshes aren't reported as unexpected misses. Within a stable window, any change that interpolates per-turn data (timestamps, frame-shifting context) into the system prompt still silently disables prompt caching for those providers.

### Deferred tools (toolSearch)

To keep the tools prefix small and byte-stable, MCP tools and non-core built-ins are not sent as schemas up front. `core/src/agent/tool-search/catalog.ts:buildDeferredCatalog` builds a boot-time catalog that the system prompt lists by NAME under a `## Deferred Tools` block (in place of the old `## MCP Tools` block), and the model pulls full schemas on demand via the `toolSearch` tool. `composeTurnTools` splices activated tools into the current turn's tool list, so sessions that never search keep a byte-stable tools prefix across turns.

Deferral is skipped — everything falls back to full injection — when the model matches `WEAK_MODEL_PATTERNS` (it may not call `toolSearch` unprompted) or when the catalog weighs less than `DEFERRAL_THRESHOLD_PERCENT` of the context window. MCP tools annotated `alwaysLoad: true` bypass deferral by server-author declaration. The experimental `experimentalToolProfile: 'standard'` user config (`AgentOptions.toolProfile`) defers an even larger built-in set, honored only for an allowlist of validated strong models. Sub-agents get no catalog and keep the inline `## MCP Tools` block.
