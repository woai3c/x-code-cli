...
// image plus a compact console-error summary. Navigation snapshots stay inside
// the process and never consume model context.
import { tool } from 'ai'
...
export const browserVisualCheck = tool({
  description: `Capture one lightweight screenshot of a local web app for visual QA after frontend changes. This uses the managed browser and its existing session, but does not run or require the opt-in multi-turn browser sub-agent. It opens an isolated temporary tab, accepts only localhost/loopback HTTP(S) URLs and rejects external redirects, then restores the user's original tab. It returns one current-viewport JPEG plus a short console-error summary and discards accessibility snapshots. Use it once before finishing a significant visual change; call it again only after fixing something visible. Use the browser sub-agent instead when clicking, typing, authentication, or other multi-step interaction is required.`,
