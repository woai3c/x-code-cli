...
describe('grep tool', () => {
  it.skipIf(!hasRg)('finds matching content in files', async () => {
    ...
    const result = await grep.execute!({ pattern: 'hello', path: tmpDir }, {
      toolCallId: 'test',
      messages: [],
  ...
  it.skipIf(!hasRg)('returns no matches message when nothing found', async () => {
    ...
    const result = await grep.execute!({ pattern: 'nonexistent_pattern_xyz', path: tmpDir }, {
      toolCallId: 'test',
      messages: [],
      abortSignal: undefined,
    } as any)
  ...
  it.skipIf(!hasRg)('supports glob filter', async () => {
    ...
    const result = await grep.execute!({ pattern: 'hello', path: tmpDir, glob: '*.ts' }, {
      toolCallId: 'test',
      messages: [],
      abortSignal: undefined,
    } as any)
  ...
  it.skipIf(!hasRg)('truncates results when exceeding headLimit', async () => {
    ...
    const result = (await grep.execute!({ pattern: 'match_target', path: tmpDir, headLimit: 5 }, {
      toolCallId: 'test',
      messages: [],
      abortSignal: undefined,
    } as any)) as string
