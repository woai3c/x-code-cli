...
3. 其他 schema version 视为不支持，在 /memory status 报错并停止自动写入，不能猜测升级。
...
Memory v2 始终启用，不提供启停配置或命令；`/memory status` 只展示运行状态。动态配置值不得插入已缓存的 system prompt；MemoryService 在 turn 边界读取配置。
...
9. 未完成任一存储、队列或召回链路时不得合并或发布；不提供部分功能模式或运行时禁用开关。
